...
static bool check_factory_reset(void) {
  ...
  gpio_reset_pin((gpio_num_t)FACTORY_RESET_PIN);
  gpio_set_direction((gpio_num_t)FACTORY_RESET_PIN, GPIO_MODE_INPUT);
  gpio_set_pull_mode((gpio_num_t)FACTORY_RESET_PIN, GPIO_PULLUP_ONLY);
  ...
  if (gpio_get_level((gpio_num_t)FACTORY_RESET_PIN) == 0) {
    ...
    int held_ms = 0;
    while (gpio_get_level((gpio_num_t)FACTORY_RESET_PIN) == 0 && held_ms < FACTORY_RESET_HOLD_MS) {
...
#else
  esp_err_t err;
  err = memory_set("wifi_ssid", WIFI_SSID);
  err = memory_set("wifi_pass", WIFI_PASS);
  err = memory_set("llm_backend", LLM_BACKEND);
  err = memory_set("api_key", LLM_API_KEY);
  err = memory_set("llm_model", LLM_MODEL);
  err = memory_set("llm_api_url", LLM_API_URL);
  err = memory_set("tg_token", TG_TOKEN);
  err = memory_set("tg_chat_id", TG_CHAT_ID);
  err = memory_set("tg_chat_ids", TG_CHAT_IDS);
  ESP_LOGE(TAG, "NVS config defaults stored");
  return false;
#endif
}
